...
export default function ServiceAreas({
  ...
  return (
    <section className="bg-white py-[clamp(56px,7vw,100px)]">
      {/* Mobile order (single column): text → image → location tiles.
          Desktop (md+): text top-left, tiles bottom-left, image spans the
          right column — achieved with explicit grid placement. */}
      <div className="mx-auto grid max-w-shell grid-cols-1 gap-7 px-5 sm:px-8 md:grid-cols-2 md:grid-rows-[auto_1fr] md:items-stretch md:gap-x-12 md:gap-y-6">
        {/* Text block */}
        ...
        {/* Image — full (uncropped) on mobile via the photo's own aspect
            ratio; on desktop it spans the right column and fills (cover). */}
        <div className="relative aspect-[2222/3326] overflow-hidden rounded-[2px] md:col-start-2 md:row-start-1 md:row-span-2 md:aspect-auto md:min-h-[420px]">
          <MediaSlot
            src={photoSrc}
        ...
        {/* Location tiles */}
        <div className="grid grid-cols-2 gap-px border border-black/10 bg-black/10 md:col-start-1 md:row-start-2 md:self-end">
          {AREA_TILES.map((a) => (
            <Link
              key={a}
